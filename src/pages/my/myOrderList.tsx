...
const MyOrderList: React.FC = () => {
  const [selectedTab, setSelectedTab] = useState<"orders" | "refunds">(
    "orders"
  );
  ...
  // cancel refund
  const [isCancelRefundModalOpen, setCancelRefundModalOpen] =
    useState<boolean>(false);
  // review
  const [isReviewModalOpen, setReviewModalOpen] = useState<boolean>(false);
  // confirm purchas
  const [isConfirmPurchasModalOpen, setConfirmPurchaseModalOpen] =
    useState<boolean>(false);
    ...
    .filter((order) => {
      ...
      if (selectedPeriod !== "전체") {
        ...
        if (startDateForPeriod) {
          dateFilter = isWithinInterval(new Date(order.date), {
            start: startDateForPeriod,
            end: currentDate,
          });
        }
      } else {
      ...
      if (startDate && endDate) {
        ...
        dateFilter = isWithinInterval(orderDate, {
          start: startDate,
          end: endDate,
        });
      }
                  ...
                  <OrderRow key={order.ordernumber}>
                    ...
                    <TableCell>
                      ...
                      <div style={{ marginTop: "5px" }}>
                        {!order.isPurchased && (
                          <Button onClick={() => handleCancelRefund(order)}>
                            취소/환불
                          </Button>
                        )}
                      </div>
                    ...
                    <TableCell>
                      {!["취소중", "취소완료"].includes(order.status) &&
                        (order.isPurchased ? (
                          ...
                          <Button onClick={() => handleConfirmPurchase(order)}>
                            구매확정
                          </Button>
                        ))}
                    </TableCell>
          ...
          {isConfirmPurchasModalOpen && selectedOrder && (
            <ModalOverlay onClick={() => setConfirmPurchaseModalOpen(false)}>
              <ModalContainer>
                ...
                <ButtonGroup>
                  <ConfirmButton bgColor="black" onClick={confirmPurchase}>
                    구매확정하기
                  </ConfirmButton>
